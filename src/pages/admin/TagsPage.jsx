...
import { useADeleteTagMutation, useAGetTagsQuery } from '../../store/index.api'
...
const TagsPage = () => {
  ...
  const { data } = useAGetTagsQuery({ page, limit: 20 })
  const [deleteTag] = useADeleteTagMutation()
  const totalPages = Math.ceil(data?.total / 20) ?? 1

  const handleDelete = (id) => deleteTag(id)
  ...
  return (
    <Card className="h-full w-full">
        ...
        <div className=" flex flex-col justify-between gap-8 md:flex-row md:items-center">
          <div>
            ...
            <Typography color="gray" className="mt-1 font-normal">
              Здесь находятся информации о тегах
            </Typography>
          </div>
      ...
      <CreateTagModal open={open} setIsOpen={setOpen} />
    </Card>
  )
}
